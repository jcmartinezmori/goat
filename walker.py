...
def main(w_mat, version, no_samples):
    ...
    while ct < no_samples:
        ...
        pi, p, p_sum = walk(pi, p, p_sum, w_mat, trans, trans_supp)
    return pis
...
def walk(pi, p, p_sum, w_mat, trans, trans_supp):
    if np.random.uniform() <= 0.5:
        p /= p_sum
        i_star, j_star = trans[np.random.choice(range(trans.shape[0]), p=p)]
        p *= p_sum
        pi[i_star], pi[j_star] = pi[j_star], pi[i_star]
        for idx, (i, j) in set.union(trans_supp[i_star],trans_supp[j_star]):
            val = w_mat[pi[j], pi[i]]
            p_sum += val - p[idx]
            p[idx] = val
